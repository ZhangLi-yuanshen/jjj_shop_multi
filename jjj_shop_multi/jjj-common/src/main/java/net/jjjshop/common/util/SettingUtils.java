...
public class SettingUtils {
     ...
     * @throws Exception
     */
    public JSONObject getSupplierSetting(String key, Long appId, Integer shopSupplierId) {
        if(appId == null){
            appId = RequestDetailThreadLocal.getRequestDetail().getAppId();
        }
        String cacheKey = String.format(CommonRedisKey.SUPPLIER_SETTING_DATA, key, appId,shopSupplierId);
        // 如果缓存中存在,则返回
        Object object = redisTemplate.opsForValue().get(cacheKey);
        if(object != null){
            //return (JSONObject)object;
