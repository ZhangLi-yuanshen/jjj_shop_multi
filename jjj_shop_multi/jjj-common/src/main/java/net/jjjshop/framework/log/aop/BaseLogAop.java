...
import net.jjjshop.common.service.supplier.SupplierLoginLogService;
...
public abstract class BaseLogAop {
    ...
    @Autowired
    protected SupplierOptLogService supplierOptLogService;
    ...
    protected void saveShopLoginLog(RequestInfo requestInfo, OperationLogInfo operationLogInfo, Object result, Exception exception) {
        try {
            ...
            String optName = "";
            // 判断是否是登录路径
            if (realPath.equals(loginLogConfig.getShopLoginPath())) {
                flag = true;
                optName = "登录成功";
            } else if (realPath.equals(loginLogConfig.getShopLogoutPath())) {
                flag = true;
                optName = "退出成功";
            }
            ...
            if (flag && result != null && result instanceof ApiResult) {
                ...
                if (apiResult.getCode() == 1) {
                    ...
                    if ("登录成功".equals(optName)) {
                        ...
                        shopLoginLog.setAppId(loginShopUserVo.getLoginShopUserVo().getAppId());
                    }else {
     ...
     */
    @Async
    protected void saveSupplierOperationLog(RequestInfo requestInfo, OperationLogInfo operationLogInfo, Object result, Exception exception) {
        try {
            Integer shopUserId = SupplierLoginUtil.getUserId();
            if(shopUserId == null){
                return;
            }
            JSONObject vo = settingUtils.getSetting(SettingEnum.STORE.getKey(), null);
            ...
            // 操作日志
            SupplierOptLog supplierOptLog = new SupplierOptLog();
            // 设置操作日志信息
            if (operationLogInfo != null) {
                // 如果类或方法上标注有OperationLogIgnore，则跳过
                if (operationLogInfo.isIgnore()) {
            ...
            // 设置请求参数信息
            if (requestInfo != null) {
                supplierOptLog.setIp(requestInfo.getIp())
                        .setUrl(requestInfo.getPath())
                        .setRequestType(requestInfo.getRequestMethod());

                // 设置参数字符串
                supplierOptLog.setContent(Jackson.toJsonStringNonNull(requestInfo.getParam()));
                // User-Agent
                String userAgent = requestInfo.getUserAgent();
                if (StringUtils.isNotBlank(userAgent)) {
                    supplierOptLog.setAgent(StringUtils.substring(userAgent, ZERO, MAX_LENGTH));
                }
                ClientInfo clientInfo = ClientInfoUtil.get(requestInfo.getUserAgent());
                if (clientInfo != null) {
                    supplierOptLog.setBrowser(clientInfo.getBrowserName());
                }
            }

            // 设置当前登录信息
            supplierOptLog.setShopSupplierId(SupplierLoginUtil.getShopSupplierId());
            supplierOptLog.setSupplierUserId(shopUserId.intValue());
            // 保存日志到数据库
            supplierOptLogService.save(supplierOptLog);
        } catch (Exception e) {
            if (e instanceof JWTDecodeException) {
                JWTDecodeException jwtDecodeException = (JWTDecodeException) e;
                throw jwtDecodeException;
     ...
     */
    @Async
    protected void saveSupplierLoginLog(RequestInfo requestInfo, OperationLogInfo operationLogInfo, Object result, Exception exception) {
        try {
            String realPath = requestInfo.getRealPath();
            if (StringUtils.isBlank(realPath)) {
                return;
            ...
            String optName = "";
            // 判断是否是登录路径
            if (realPath.equals(loginLogConfig.getSupplierLoginPath())) {
                flag = true;
                optName = "登录成功";
            } else if (realPath.equals(loginLogConfig.getSupplierLogoutPath())) {
                flag = true;
                optName = "退出成功";
            }
            ...
            if (flag && result != null && result instanceof ApiResult) {
                ...
                if (apiResult.getCode() == 1) {
                    ...
                    supplierLoginLog.setResult(optName);
                    supplierLoginLog.setIp(requestInfo.getIp());
                    if ("登录成功".equals(optName)) {
                        LoginSupplierUserTokenVo loginSupplierUserTokenVo = (LoginSupplierUserTokenVo)apiResult.getData();
                        JSONObject paramObject = (JSONObject)requestInfo.getParam();
                        supplierLoginLog.setUsername(paramObject.getString("username"));
                        supplierLoginLog.setAppId(loginSupplierUserTokenVo.getLoginSupplierUserVo().getAppId());
                        supplierLoginLog.setShopSupplierId(loginSupplierUserTokenVo.getLoginSupplierUserVo().getShopSupplierId());
                    }else {
                        String username = JwtUtil.getUsername(requestInfo.getToken());
                        supplierLoginLog.setUsername(username);
                        RequestDetail requestDetail = RequestDetailThreadLocal.getRequestDetail();
                        supplierLoginLog.setShopSupplierId(requestDetail.getShopSupplierId());
                    }
                    // 保存登录日志
                    supplierLoginLogService.save(supplierLoginLog);
                }
            }
        } catch (Exception e) {
            log.error("保存supplier登录日志失败", e);
        }
    }
