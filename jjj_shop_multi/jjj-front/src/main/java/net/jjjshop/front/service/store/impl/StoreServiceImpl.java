...
public class StoreServiceImpl extends BaseServiceImpl<StoreMapper, Store> implements StoreService {
    ...
    private List<StoreVo> transToVoList(List<Store> list){
        // 转成vo
        return list.stream().map(this::transToVo).collect(Collectors.toList());
    }
    ...
    private StoreVo transToVo(Store store){
        ...
        vo.setCity(regionService.getById(store.getCityId()).getName());
        vo.setRegion(regionService.getById(store.getRegionId()).getName());
        return vo;
    }
     ...
     * @return
     */
    public List<StoreVo> getAllCheck(String longitude, String latitude, Integer shopSupplierId){
        List<Store> list = this.list(new LambdaQueryWrapper<Store>().eq(Store::getIsCheck, 1)
                .eq(Store::getStatus, 1).eq(Store::getShopSupplierId, shopSupplierId)
                .orderByAsc(Store::getSort).orderByDesc(Store::getCreateTime));
