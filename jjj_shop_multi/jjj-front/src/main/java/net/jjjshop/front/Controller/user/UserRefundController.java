...
public class UserRefundController extends BaseController {
    ...
    @RequestMapping(value = "/plateApply", method = RequestMethod.POST)
    @OperationLog(name = "plateApply")
    @ApiOperation(value = "plateApply", response = String.class)
    public ApiResult<String> plateApply(Integer orderRefundId){
        if(orderRefundService.plateApply(orderRefundId)){
            return ApiResult.ok(null, "申请平台售后成功");
        }else{
            return ApiResult.fail("申请平台售后失败");
        }
    }
