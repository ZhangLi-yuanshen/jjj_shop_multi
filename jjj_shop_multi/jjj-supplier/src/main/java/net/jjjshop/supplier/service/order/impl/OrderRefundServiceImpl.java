...
import net.jjjshop.supplier.vo.order.OrderProductVo;
...
public class OrderRefundServiceImpl extends BaseServiceImpl<OrderRefundMapper, OrderRefund> implements OrderRefundService {
     ...
     * @return
     */
    public Paging<OrderRefundVo> getList(OrderRefundPageParam orderRefundPageParam, Integer shopSupplierId) {
     ...
     * @return
     */
    public Integer getCountByStatus(OrderRefundPageParam orderRefundPageParam, Integer shopSupplierId) {
        LambdaQueryWrapper<OrderRefund> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(OrderRefund::getShopSupplierId, shopSupplierId);
        //根据订单号模糊搜索
        if (StringUtils.isNotEmpty(orderRefundPageParam.getOrderNo())) {
    ...
    public Boolean audit(OrderRefundParam orderRefundParam) {
        ...
        messageUtils.refund(orderRefund, order.getOrderNo());
        //如果是仅退款
        if (orderRefundParam.getIsAgree() == 10 && orderRefund.getType() == 30) {
     ...
     * @return
     */
    public OrderRefundVo detail(Integer orderRefundId, Integer shopSupplierId) {
        ...
        vo.setTypeText(this.getTypeAttr(vo.getType()));
        if (vo.getIsAgree() == 0) {
            vo.setAddressList(returnAddressService.getAll(shopSupplierId));
     ...
     * 已同意的退款
     */
    public BigDecimal getRefundMoney(Integer shopSupplierId){
        QueryWrapper<OrderRefund> wrapper = new QueryWrapper<>();
        wrapper.lambda().eq(OrderRefund::getIsAgree, 10);
        wrapper.lambda().eq(OrderRefund::getShopSupplierId, shopSupplierId);
