package net.jjjshop.supplier.service.product.impl;
...
public class ProductServiceImpl extends BaseServiceImpl<ProductMapper, Product> implements ProductService {
    ...
    public Map<String,Object> getList(ProductPageParam productPageParam){
        ...
        wrapper.eq(Product::getShopSupplierId, productPageParam.getShopSupplierId());
        ...
        //出售中
        productCount.put("sell", this.getCount("sell", productPageParam.getShopSupplierId()));
        //仓库中
        productCount.put("lower", this.getCount("lower", productPageParam.getShopSupplierId()));
        //回收站
        productCount.put("recovery", this.getCount("recovery", productPageParam.getShopSupplierId()));
        //库存紧张
        productCount.put("stock", this.getCount("stock", productPageParam.getShopSupplierId()));
        //库存紧张
        productCount.put("audit", this.getCount("audit", productPageParam.getShopSupplierId()));
        //已售罄
        productCount.put("noAudit", this.getCount("noAudit", productPageParam.getShopSupplierId()));
        //已售罄
        productCount.put("draft", this.getCount("draft", productPageParam.getShopSupplierId()));
        result.put("productCount", productCount);
        // 商店状态
        result.put("supplierStatus", supplierService.getById(productPageParam.getShopSupplierId()).getStatus());
        // 商品分类
     ...
     * @return
     */
    private Integer getCount(String type, Integer shopSupplierId){
        LambdaQueryWrapper<Product> wrapper = new LambdaQueryWrapper<>();
    ...
    private void setWrapperByType(LambdaQueryWrapper<Product> wrapper, String type){
        ...
        if (type.equalsIgnoreCase("lower")) {
            wrapper.eq(Product::getProductStatus, 20).eq(Product::getAuditStatus, 10);
        }
        // 回收站
        ...
        if (type.equalsIgnoreCase("stock")) {
            wrapper.ge(Product::getProductStock, 1).le(Product::getProductStock, 10)
                    .eq(Product::getProductStatus, 10).eq(Product::getAuditStatus, 10);
        }
        //待审核
        ...
        if (type.equalsIgnoreCase("noAudit")) {
            wrapper.eq(Product::getAuditStatus, 20);
        }
        //草稿
        if (type.equalsIgnoreCase("draft")) {
            wrapper.eq(Product::getAuditStatus, 40);
        }
    }
    ...
    public boolean add(ProductParam productParam){
        ...
        JSONObject setting = settingUtils.getSetting(SettingEnum.STORE.getKey(), null);
        StoreVo storeVo = setting.toJavaObject(StoreVo.class);
        if(storeVo.getAddAudit() == 0) {
            // 如果不需要审核，则审核状态是已审核
            product.setAuditStatus(10);
        }
        //记录商品的供应商Id值
        Integer userId = SupplierLoginUtil.getUserId();
        SupplierUser user = supplierUserService.getById(userId);
        product.setShopSupplierId(user.getShopSupplierId());
        // 保存商品
        this.save(product);
        // 保存商品规格
    ...
    public Integer getProductTotal(Integer shopSupplierId){
        return this.count(new LambdaQueryWrapper<Product>().eq(Product::getIsDelete, 0).eq(Product::getShopSupplierId, shopSupplierId));
    }
    ...
    public Integer getProductStockTotal(Integer shopSupplierId) {
        return this.count(new LambdaQueryWrapper<Product>()
                .eq(Product::getShopSupplierId, shopSupplierId)
                .eq(Product::getIsDelete, 0)
                .lt(Product::getProductStock, 20));
    }
}
