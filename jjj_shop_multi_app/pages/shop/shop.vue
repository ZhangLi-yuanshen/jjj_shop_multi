		...
		<view class="shop_head">
			...
			<view class="shop_head_info">
				<view class="shop_list_body_item_shop">
					<view class="shop_list_body_item_shop_logo">
						<image :src="shop_info.logoFilePath" mode=""></image>
					</view>
					<view class="shop_list_body_item_shop_info">
						...
						<view class="d-s-c">
							<view class="f22 white sales">销量:{{shop_info.productSales ||''}}件</view>
							<view class="f22 white ml30 collect" style="text-align: center;">
								关注:<text>{{shop_info.favCount ||''}}</text></view>
						</view>

					</view>
					<view class="shop_list_body_item_shop_others">
						<view class="f24 white collect mb30">店铺评分：<text>{{shop_info.serverScore ||''}}</text></view>
			...
			<view class="inner-tab" v-if="nav_type==0">
				...
				<view class="item" @click="select_type()">
					<view class="box">
						<image :src="isLieBiao == true?'/static/shop/liebiao.png':'/static/shop/tubiao.png'"></image>
					</view>
				</view>
			</view>
			<!-- 商品导航栏 -->
		</view>
		<scroll-view v-if="nav_type!=2" scroll-y="true" :style="'height:' + scrollviewHigh + 'rpx;'"
			class="scroll-Y pr bg-f2" lower-threshold="50" @scrolltolower="scrolltolowerFunc">
			<view class="shop" v-if="shop_info!=''">
				<view class="shop_body" v-if="isLieBiao ==true&&nav_type==0">
					<view class="shop_body_l_item" :class="index==product_list.length-1?'noborder':''"
						v-for="(item,index) in product_list" :key="index" @click="goto_product(item.productId)">
						<view>
					...
					<view class="shop_body_t_item" v-for="(item,index) in product_list" :key="index"
						...
						<view class="shop_body_t_item_info">
							...
							<view class="shop_body_t_item_info_price">
								<view class="f24 redF6">¥<text class="h1">{{item.productPrice}}</text></view>
								<view class="f24 huaxianjia">¥<text class="h3">{{item.linePrice}}</text></view>
							</view>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			self.shop_supplier_id = option.shopSupplierId;
		},
		onShow() {
        ...
        this.gotoPage(linkUrl);
			...
			getProduct(e) {
				...
				self._postBody('product/product/lists', {
					...
					sortType: e,
					type: "sell",
					shopSupplierId: self.shop_supplier_id,
				}, function(res) {
					self.loading = false;
					self.product_list = [...self.product_list, ...res.data.records];
			...
			getData() {
				let self = this;
				self.loading = true;
				self._postBody('supplier/index/index', {
					shopSupplierId: self.shop_supplier_id,
      ...
      goto_zhu() {
        this.gotoPage('pages/index/index');
			...
			guanzhu() {
				let self = this;
				self._postBody('user/favorite/add', {
