				...
				<view class="item" v-for="(item, index) in listData" :key="index">
					<view class="d-b-c pb38">
						<view class="item-dianpu" @click="toShop(item.shopSupplierId)">
							<view class="item-d-l mr10">
								...
								<text class="fb gray3 f32" v-if="item.shopSupplierId > 0">{{ item.supplierName }}</text>
							</view>
							<view><i class="icon iconfont icon-jiantou"></i></view>
						</view>
						<view class="state">
							<text class="redF6">{{ item.stateText }}</text>
						</view>
					</view>
					...
					<view class="order-bts">
						<block v-if="item.orderStatus == 10">
							...
							<!-- 订单核销码 -->
							<block v-if="item.payStatus == 20 && item.deliveryType == 20 && item.deliveryStatus == 10">
