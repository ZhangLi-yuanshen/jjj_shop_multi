<template>
	<view class="refund-list">
		<view class="top-tabbar">
			<view :class="state_active == -1 ? 'tab-item active' : 'tab-item'" @click="stateFunc(-1)">全部</view>
			...
			<view class="list">
				<view class="item bg-white p30 mb20" v-for="(item, index) in tableData" :key="index">
					...
					<view class="one-product d-s-c pt20">
						<view class="cover"><image :src="item.orderProduct.productImage" mode="aspectFit"></image></view>
						<view class="flex-1">
							<view class="pro-info">{{ item.orderProduct.productName }}</view>
							<view class="pt10 p-0-30">
								<text class="f24 gray9"><!--斤:10斤; 颜色:红; 出产地:北方; 销量:大于1000; 大小:直径1分米; 口感:甜;--></text>
							</view>
						</view>
					</view>
					...
					<view class="d-e-c mt20 pt20 border-t">
						...
						<button type="default" class="btn-gray-border" @click="gotoRefundDetail(item.orderRefundId)">查看详情</button>
					</view>
				</view>
