			...
			<view class="address-list bg-white">
				<scroll-view scroll-y="true" class="specs mt20" style="max-height: 600rpx;">
					<view   class="p20 address-item" v-for="(item,index) in storeList" :key="index">
						<view   :class="item.storeId == selectedId?'active':''" class="address  d-s-c">
...
<script>
	export default {
		...
		props: ['isAddress','storeId','chooseSotr'],
		watch:{
			isAddress(val){
				this.Visible=val;
				if(val){
					...
					//#endif
					// 记录已选中的id
					this.selectedId = this.$props.storeId;
