<template>
	<view class="product-detail pr" :data-theme="theme()" :class="theme() || ''">
		...
		<scroll-view scroll-y="true" class="scroll-Y scroll-box" :style="'height:' + scrollviewHigh + 'px'" v-if="!loadding">
			...
			<view class="product-pic">
				...
				>
					...
					<swiper-item v-for="(item, index) in detail.image" :key="index"><image :src="item.filePath" mode="aspectFit"></image></swiper-item>
				</swiper>
			</view>
			<!--基本信息-->
			<template v-if="ispresale">
				...
				<view class="limited-spike m-0-20">
					<view class="d-s-c mb16">
						<view class="f32">
							￥
							<text class="f42 fb">{{ subPrice(detail.advance.sku[0].product_price, '1') }}.</text>
							<text class="f24 fb">{{ subPrice(detail.advance.sku[0].product_price, '2') }}</text>
						</view>
						<view class="tips-box">预估到手价￥{{ (detail.advance.sku[0].product_price * 1 - detail.advance.sku[0].advance_price * 1).toFixed(2) }}</view>
					</view>
					...
					<view class="right-time">
						...
						<Countdown ref="countdown" :config="{ startstamp: detail.advance.start_time, endstamp: detail.advance.end_time }" @returnVal="returnValFunc"></Countdown>
					</view>
				</view>
					...
					<view class="product-presale">
						<view class="d-b-s">
							...
							<view class="flex-1">
								...
								<view class="gray9 f22">({{ detail.advance.active_time[0] }}-{{ detail.advance.active_time[1] }})</view>
							</view>
						</view>
						<view class="d-b-s">
			...
			<view class="bg-white p30 mb22" v-if="!ispresale">
					...
					<view class="d-s-s d-c ww100">
						<view class="d-s-c pr ww100 mb16">
							<view class="new-price theme-price">
								...
								<text v-if="detail.specType == 20">-{{ detail.highPrice }}</text>
							</view>
							...
							<view class="sc-box">
								<button @click="favorite()">
									<image :class="isFav ? '' : 'img_gray'" class="share_img" src="/static/icon/sc.png" mode=""></image>
				...
				<view class="product-name text-ellipsis-2">{{ detail.productName }}</view>
					...
					<view class="d-b-c">
						<view class="flex-1">
							<view class="pb20 line-h-50 f22 gray3 " v-if="discount.give_points > 0">
								<text class="text-box">返积分</text>
								商城购物返积分，订单完成后最高返{{ discount.give_points }}积分
							</view>
								...
								<view class="f22 gray3 line-h-50 pb20 flex-1">
									<view class=" d-b-s">
										...
										<view class="flex-1 text-coupon-list">
											<view
												class="ml20 mb10 text-coupon-box "
			...
			<view class="shop_head_info" v-if="store_open">
				<view class="shop_list_body_item_shop">
					...
					<view class="shop_list_body_item_shop_info flex-1">
						...
						<view class="f26 brand gray9">主营品牌： {{ shop_info.categoryName }}</view>
						<view class="f26 sales gray9">销量：{{ shop_info.productSales }}件</view>
					</view>
					<view class="shop_list_body_item_shop_others">
						<view class="f26 collect">
							店铺评分：
							<text class="fb">{{ shop_info.serverScore }}</text>
						</view>
			...
			<view class="product-content">
				...
				<view v-if="detail.isPicture == 1" class="content-box">
					<view class="ww100" v-for="(item, index) in detail.contentImage" :key="index"><image class="ww100" :src="item.filePath" mode="widthFix"></image></view>
				</view>
			</view>
		</scroll-view>
		...
		<view class="btns-wrap">
			...
			<template>
				<button v-if="!isVirtual" class="add-cart" @click="openPopup('card')">加入购物车</button>
...
export default {
	...
	methods: {
		...
		getData() {
			...
			self._get(
				...
				function(res) {
					...
					self.shop_supplier_id = res.data.detail.supplier.shopSupplierId;
					...
					self.store_open = res.data.storeOpen;
		...
		goto_shop() {
			let self = this;
			self.gotoPage('/pages/shop/shop?shopSupplierId=' + self.shop_supplier_id);
		},
