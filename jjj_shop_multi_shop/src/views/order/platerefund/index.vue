    ...
    <div class="product-content">
      <div class="table-wrap">
        <el-tabs v-model="activeName" @tab-change="handleClick">
          ...
          <el-tab-pane :label="'进行中(' + (arr[0] ) + ')'" name="10"></el-tab-pane>
          <el-tab-pane :label="'已拒绝(' + (arr[1] ) + ')'" name="30"></el-tab-pane>
          <el-tab-pane :label="'已同意(' + (arr[2] ) + ')'" name="20"></el-tab-pane>
          <el-tab-pane :label="'已完成(' + (arr[3] ) + ')'" name="40"></el-tab-pane>
        ...
        <el-table :data="tableData" border style="width: 100%" v-loading="loading">
          <el-table-column prop="orderproduct.productName" width="400" label="商品信息">
            <template  #default="scope">
              <div class="d-s-c">
                <img v-img-url="scope.row.orderProduct.imagePath" alt="" width="50px" />
          ...
          <el-table-column prop="orderproduct.productPrice" label="单价">
            <template  #default="scope">
              <span class="orange">{{ scope.row.orderProduct.productPrice }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="" label="处理状态" width="130">
            <template  #default="scope">
              ...
              <p>
                平台审核：
                <span class="orange" v-if="scope.row.plateStatus == 10">{{ scope.row.plateStatusText }}</span>
                <span class="orange" v-if="scope.row.plateStatus == 20">{{ scope.row.plateStatusText }}</span>
                <span class="orange" v-if="scope.row.plateStatus == 30">{{ scope.row.plateStatusText }}</span>
              </p>
              <p>
                商家审核：
                <span class="orange" v-if="scope.row.status == 0">{{ scope.row.statusText }}</span>
                <span class="orange" v-if="scope.row.status == 10">{{ scope.row.statusText }}</span>
                <span class="orange" v-if="scope.row.status == 20">{{ scope.row.statusText }}</span>
              </p>
              <!-- 发货状态-->
              <p v-if="scope.row.type == 10 && scope.row.plateStatus == 20">
                用户发货：
              ...
              <!-- 商家收货状态-->
              <p
                v-if="scope.row.type == 10 && scope.row.plateStatus == 20 && scope.row.isUserSend  == 1 && scope.row.isReceipt == 0">
                <span>商家收货：</span>
          ...
          <el-table-column fixed="right" label="操作" width="80">
            <template  #default="scope">
              <p>
                <el-button @click="Detail(scope.row.orderRefundId)" type="text" size="small">售后详情</el-button>
              </p>
              <p>
                <el-button v-if="scope.row.plateStatus == 10" @click="Detail(scope.row.orderRefundId)"
  ...
  export default {
    ...
    methods: {
      ...
      getTableList(params = '') {
        ...
        let Params = {};
        self.arr = [0,0,0,0];
        Params.plateStatus = self.plateStatus;
