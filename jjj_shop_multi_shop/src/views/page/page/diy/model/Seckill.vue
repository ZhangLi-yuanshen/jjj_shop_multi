<template>
  ...
  <div class="drag optional" :class="{ selected: index === selectedIndex }"
    ...
    <div class="diy-seckill">
      <div class="diy-head d-b-c" :style="'background: url('+item.style.backgroundImage+');'">
        ...
        <li class="product-item" v-for="(product, index) in item.data" :key="index">
          ...
          <div class="product-info p-0-10">
            <div class="price  f12 tc">
              <span v-if="item.style.show.seckillPrice" class="red">¥120</span>
              <span v-if="item.style.show.linePrice" class="gray6 text-d-line">¥233</span>
            </div>
          </div>
        </li>
      </ul>
