<template>
  <div class="product-add" v-loading="loading">
    ...
    <el-form size="small" ref="form" :model="form" label-width="180px" v-if="!loading">
      ...
      <!--审核-->
      <div class="common-button-wrapper" :style="oldAudit == 0 || oldAudit == 20 ? 'height: 100px;' : 'height: 50px'">
        <Audit v-if="oldAudit == 0 || oldAudit == 20"></Audit>
        <el-button size="small" type="info" @click="cancelFunc">取消</el-button>
        <el-button size="small" type="primary" @click="onSubmit('edit')" :disabled="save_loading" v-if="oldAudit == 10">发布</el-button>
        <el-button size="small" type="primary" @click="changeAudit(form.model.productId, form.model.auditStatus)" :disabled="save_loading" v-else>审核</el-button>
      </div>
    </el-form>
  </div>
</template>
  ...
  methods: {
    ...
    getData: function() {
      ...
      PorductApi.getEditData(
        ...
        .then(res => {
          self.loading = false;
          self.oldAudit = res.data.model.auditStatus;
          /*单独产品折扣*/
          if (res.data.model.aloneGradeEquityJson != null && typeof res.data.model.aloneGradeEquityJson != 'undefined') {
            ...
            res.data.gradeList.forEach(item => {
              item.productEquity = equitys[item.id];
            });
          }
          Object.assign(self.form, res.data);
          if (res.data.specData) {
            self.$set(self.form.model, 'specMany', res.data.specData);
          } else {
            self.$set(self.form.model, 'specMany', {
              specAttr: [],
              specList: []
            });
