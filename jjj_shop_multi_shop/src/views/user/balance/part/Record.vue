    ...
    <div class="product-content">
      <div class="table-wrap">
        <el-table size="small" :data="tableData" border style="width: 100%" v-loading="loading">
          ...
          <el-table-column prop="realMoney" label="到账数量"></el-table-column>
          <el-table-column label="充值方式">
            <template #default="scope">
              <span v-if="scope.row.type && scope.row.type == 10">自定义金额</span>
              <span v-if="scope.row.type && scope.row.type == 20" class="green">套餐充值</span>
            </template>
          </el-table-column>
          <el-table-column label="付款状态">
            <template #default="scope">
              <span v-if="scope.row.payStatus && scope.row.payStatus == 10">待付款</span>
              <span v-if="scope.row.payStatus && scope.row.payStatus == 20" class="green">已付款</span>
            </template>
          </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      getTableList() {
        ...
        PlanApi.log(Params, true).then(res => {
          self.loading = false;
