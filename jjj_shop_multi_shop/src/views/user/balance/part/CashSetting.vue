<template>
  ...
  <div class="pb50">
    <el-form ref="form" size="small" :model="form" label-width="200px">
      ...
      <el-form-item label="提现比例 " prop="cashRatio" :rules="[{required: true,message: ' '}]">
        <el-input placeholder="请输入内容" v-model="form.cashRatio" class="max-w460">
      ...
      <el-form-item label="最低提现金额" prop="minMoney" :rules="[{required: true,message: ' '}]">
        <el-input placeholder="请输入内容" v-model="form.minMoney" class="max-w460">
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        let self = this;
        let Params = {};
        BalanceApi.getCashSetting(Params, true).then(res => {
          self.form = res.data;
        }).catch(error => {
