<template>
  ...
  <div class="pb50">
    <el-form ref="form" size="small" :model="form" label-width="200px">
      ...
      <el-form-item label="积分名称 " prop="pointsName" :rules="[{required: true,message: ' '}]">
        <el-input v-model="form.pointsName" autocomplete="off" class="max-w460"></el-input>
      ...
      <el-form-item label="积分赠送比例 " prop="giftRatio" :rules="[{required: true,message: ' '}]">
        <el-input placeholder="请输入内容" v-model="form.giftRatio" class="max-w460">
...
<!--      <el-form-item label=" 是否允许积分转换余额 ">
        <el-radio-group v-model="form.isTransBalance" class="max-w460">
          <el-radio :label="1">允许</el-radio>
          <el-radio :label="0">不允许</el-radio>
        </el-radio-group>
        <div class="lh18 mt10 gray9">
      ...
      <el-form-item label=" 积分抵扣比例">
        <el-input placeholder="请输入内容" v-model="form.discount.discountRatio" class="max-w460">
      ...
      <el-form-item label=" 抵扣条件">
        <el-input placeholder="请输入内容" v-model="form.discount.fullOrderPrice" class="max-w460">
          <template slot="prepend">订单满</template>
          <template slot="append">元</template>
        </el-input>
      </el-form-item>
      <el-form-item label=" ">
        <el-input placeholder="请输入内容" v-model="form.discount.maxMoneyRatio" class="max-w460">
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        let self = this;
        let Params = {};
        PointsApi.getPoints(Params, true).then(res => {
          self.form = res.data;
          self.form.isShoppingGift = parseInt(res.data.isShoppingGift) || 1;
          self.form.isShoppingDiscount = parseInt(res.data.isShoppingDiscount)|| 1;
          self.form.isTransBalance = parseInt(res.data.isTransBalance)|| 0;
        }).catch(error => {
