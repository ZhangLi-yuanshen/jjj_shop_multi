  ...
  methods: {
    ...
    getData() {
      ...
      IndexApi.base(true)
        .then(res => {
          self.loading = false;
          self.supplier_name = res.data.supplierName;
          if(res.data.supplierBgImg != '' && res.data.supplierBgImg != null){
            self.bgimg_url = res.data.supplierBgImg;
          }else{
        ...
        if (valid) {
          ...
          UserApi.login(Params, true)
            .then(res => {
              ...
              if (res.code == 1) {
                /*保存个人信息*/
                setCookie('userinfo_supplier', res.data.loginSupplierUserVo.userName);
                setCookie('token_supplier', res.data.token);
                /*设置一个登录状态*/
