        ...
        >
          ...
          >
            <template #default="scope">
              <div class="d-s-c">
                <img
                  v-img-url="scope.row.orderProduct.imagePath"
                  alt=""
                  :width="50"
                />
          ...
          <el-table-column prop="" label="处理状态" width="130">
            <template #default="scope">
              <div v-if="scope.row.status == 0">
                ...
                <p>
                  商家审核：
                  <span class="orange" v-if="scope.row.isAgree == 0">{{
                    scope.row.isAgreeText
                  }}</span>
                  <span class="orange" v-if="scope.row.isAgree == 10">{{
                    scope.row.isAgreeText
                  }}</span>
                  <span class="orange" v-if="scope.row.isAgree == 20">{{
                    scope.row.isAgreeText
                  }}</span>
                </p>
                <!-- 发货状态-->
                <p v-if="scope.row.type != 30 && scope.row.isAgree == 10">
                  用户发货：
                  <span class="orange" v-if="scope.row.isUserSend == 0"
                    >待发货</span
                  >
                ...
                <p
                  v-if="
                    scope.row.type != 30 &&
                    scope.row.isAgree == 10 &&
                    scope.row.is_user_send == 1 &&
                    scope.row.is_receipt == 0
                  "
                >
                  <span>商家收货：</span>
          ...
          <el-table-column fixed="right" label="操作" width="120">
            <template #default="scope">
              <p>
                <el-button
                  @click="Detail(scope.row.orderRefundId)"
                  type="text"
                  size="small"
                  >详情</el-button
                >
