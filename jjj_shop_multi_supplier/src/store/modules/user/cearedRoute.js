  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (item.isRoute == 1) {
      let obj = {
        ...
        component: item.parentId===0? Main:loadView(item.path)
      }
      /*判断是否重定向*/
      if(item.redirectName && typeof(item.redirectName) != 'undefined' && item.redirectName != ''){
        obj['redirect'] = item.redirectName;
      }
      /*判断子路径是否是数组*/
